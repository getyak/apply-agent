...
3. **Load ONLY what's needed** (context budget):
   ...
   - For Infra tasks: `infra/AGENTS.md` + relevant workflow file
...
- **Respect project constraints** (from AGENTS.md):
  - Hybrid backend: TS API ↔ Python agents via HTTP only, never mix
  - LLM via OpenRouter (OPENROUTER_BASE_URL + ChatOpenAI), not Codex API
